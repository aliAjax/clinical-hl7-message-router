...
func serve(connection net.Conn, code string, delay time.Duration) {
	...
	for {
		...
		if previousControlID != "" {
			ackControlID = previousControlID
		}
		previousControlID = controlID
		messageType := application.SafeType(message)
		ack := fmt.Sprintf("MSH|^~\\&|SIMULATOR|TARGET|ROUTER|GW|%s||ACK^%s|ACK-%s|P|2.5\rMSA|%s|%s\r", time.Now().Format("20060102150405"), messageType, ackControlID, code, ackControlID)
