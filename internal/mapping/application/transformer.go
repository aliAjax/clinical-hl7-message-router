...
func (t *Transformer) apply(message *hl7.Message, operation domain.Operation) error {
	...
	if operation.CodeTable != "" {
		...
		if !ok {
			return fmt.Errorf("code table %s has no entry for %q", operation.CodeTable, value)
		}
		value = replacement
	}
	if operation.Mask {
...
func cloneMessage(message hl7.Message) hl7.Message {
	...
	for i, sourceSegment := range message.Segments {
		...
		clone.Segments[i].Fields = make([]hl7.Field, len(sourceSegment.Fields))
		for j, sourceField := range sourceSegment.Fields {
			clone.Segments[i].Fields[j] = sourceField
			clone.Segments[i].Fields[j].Repetitions = append([]string(nil), sourceField.Repetitions...)
			clone.Segments[i].Fields[j].Components = append([]string(nil), sourceField.Components...)
		}
	}
	return clone
}
