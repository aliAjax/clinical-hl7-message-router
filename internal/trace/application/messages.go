...
func (s *MemoryMessageStore) Get(ctx context.Context, id string) (MessageRecord, hl7.Message, error) {
	if err := ctx.Err(); err != nil {
		return MessageRecord{}, hl7.Message{}, fmt.Errorf("load message %s: %v", id, err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
	entry, ok := s.entries[id]
	if !ok {
		return MessageRecord{}, hl7.Message{}, fmt.Errorf("message %s: %v", id, ErrMessageNotFound)
	}
